package com.example.cinematch.ui.results

import android.os.Bundle
import androidx.lifecycle.SavedStateHandle
import androidx.navigation.NavArgs
import java.lang.IllegalArgumentException
import kotlin.String
import kotlin.jvm.JvmStatic

public data class ResultsFragmentArgs(
  public val genre: String,
) : NavArgs {
  public fun toBundle(): Bundle {
    val result = Bundle()
    result.putString("genre", this.genre)
    return result
  }

  public fun toSavedStateHandle(): SavedStateHandle {
    val result = SavedStateHandle()
    result.set("genre", this.genre)
    return result
  }

  public companion object {
    @JvmStatic
    public fun fromBundle(bundle: Bundle): ResultsFragmentArgs {
      bundle.setClassLoader(ResultsFragmentArgs::class.java.classLoader)
      val __genre : String?
      if (bundle.containsKey("genre")) {
        __genre = bundle.getString("genre")
        if (__genre == null) {
          throw IllegalArgumentException("Argument \"genre\" is marked as non-null but was passed a null value.")
        }
      } else {
        throw IllegalArgumentException("Required argument \"genre\" is missing and does not have an android:defaultValue")
      }
      return ResultsFragmentArgs(__genre)
    }

    @JvmStatic
    public fun fromSavedStateHandle(savedStateHandle: SavedStateHandle): ResultsFragmentArgs {
      val __genre : String?
      if (savedStateHandle.contains("genre")) {
        __genre = savedStateHandle["genre"]
        if (__genre == null) {
          throw IllegalArgumentException("Argument \"genre\" is marked as non-null but was passed a null value")
        }
      } else {
        throw IllegalArgumentException("Required argument \"genre\" is missing and does not have an android:defaultValue")
      }
      return ResultsFragmentArgs(__genre)
    }
  }
}
